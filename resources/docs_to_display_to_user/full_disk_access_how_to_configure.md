...
> you’ll quit the terminal and the current run of Hypervisor. After relaunching the terminal, just re-start the Hypervisor with either (a) `make run-hypervisor` (if running GenoMac-system for the first time) or (b) `just run-hypervisor`.
